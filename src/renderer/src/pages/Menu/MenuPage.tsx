...
import { Button, Input, Label, Badge, Spinner, EmptyState, Card, Textarea } from '../../components/ui/index'
import { Modal, ConfirmDialog, showToast } from '../../components/ui/Modal'
import { Plus, Pencil, Trash2, Tag, ChevronDown, ChevronUp, GripVertical } from 'lucide-react'
import type { Category, Product, ModifierGroup, Modifier } from '../../types'

/* ─── Category Form ─────────────────────────────────────────────────── */
const COLORS = ['#ef4444', '#f97316', '#eab308', '#22c55e', '#06b6d4', '#3b82f6', '#a855f7', '#ec4899', '#78716c']
const ICONS = ['🍔', '🍕', '🍟', '🥤', '🍦', '📦', '🌮', '🥗', '🍰', '☕', '🧆', '🍗']
  ...
  return (
    <Modal open onClose={onClose} title={initial?.id ? 'Edit Category' : 'New Category'} size="sm">
      <div className="p-6 space-y-4">
        <div><Label>Name</Label><Input value={name} onChange={(e) => setName(e.target.value)} placeholder="Category name" autoFocus /></div>
        <div>
          <Label>Icon</Label>
        ...
        <div><Label>Sort Order</Label><Input type="number" value={sort} onChange={(e) => setSort(e.target.value)} /></div>
        <div className="flex justify-end gap-2 pt-2">
          <Button variant="outline" onClick={onClose}>Cancel</Button>
          <Button onClick={() => name && onSave({ name, color, icon, sort_order: parseInt(sort) || 0 })}>Save</Button>
        </div>
      </div>
    </Modal>
  )
...
function ProductForm({ initial, categories, onSave, onClose }: ProductFormProps): JSX.Element {
  ...
  const [isActive, setIsActive] = useState<boolean>(initial?.is_active !== undefined ? initial.is_active : initial?.active !== undefined ? !!initial.active : true)
  return (
    <Modal open onClose={onClose} title={initial?.id ? 'Edit Product' : 'New Product'} size="lg">
      <div className="p-6 grid grid-cols-2 gap-4">
        <div className="col-span-2">
          <Label>Product Name *</Label>
          <Input value={name} onChange={(e) => setName(e.target.value)} placeholder="e.g. Chicken Burger" autoFocus />
        </div>
        <div>
          <Label>Category *</Label>
          <select className="w-full rounded-lg border bg-background px-3 py-2 text-sm" value={catId} onChange={(e) => setCatId(Number(e.target.value))}>
        ...
        <div className="flex items-center gap-2">
          <input type="checkbox" id="track" checked={trackStock} onChange={(e) => setTrackStock(e.target.checked)} className="h-4 w-4" />
        ...
        <div className="col-span-2 flex justify-end gap-2 pt-2">
          <Button variant="outline" onClick={onClose}>Cancel</Button>
          <Button onClick={() => name && price && catId && onSave({
            name, category_id: catId as number, price: parseFloat(price),
            description: desc || null, sku: sku || null, image_path: null,
            active: isActive ? 1 : 0, track_inventory: trackStock ? 1 : 0
          })}>Save Product</Button>
        </div>
      </div>
    </Modal>
...
export function MenuPage(): JSX.Element {
  ...
  const [catForm, setCatForm] = useState<{ open: boolean; initial?: Partial<Category> }>({ open: false })
  const [prodForm, setProdForm] = useState<{ open: boolean; initial?: Partial<Product> }>({ open: false })
  const [deleteTarget, setDeleteTarget] = useState<{ type: 'category' | 'product'; item: Category | Product } | null>(null)

  const load = useCallback(async () => {
    const [cats, prods] = await Promise.all([window.api.categories.getAll() as Promise<Category[]>, window.api.products.getAll(true) as Promise<Product[]>])
    setCategories(cats)
    setProducts(prods)
  ...
  const handleSaveCategory = async (data: { name: string; color: string; icon: string; sort_order: number }): Promise<void> => {
    ...
    <div className="flex h-full overflow-hidden">
      ...
      <div className="w-56 border-r flex flex-col bg-muted/20">
        <div className="flex items-center justify-between px-4 py-3 border-b">
          ...
          <button onClick={() => setCatForm({ open: true })} className="p-1 rounded hover:bg-accent">
          ...
          {categories.map((cat) => (
            ...
            >
              ...
              <div className="invisible group-hover:visible flex gap-0.5">
                <button onClick={(e) => { e.stopPropagation(); setCatForm({ open: true, initial: cat }) }} className="p-1 rounded hover:bg-accent">
                  <Pencil className="h-3 w-3" />
                </button>
                <button onClick={(e) => { e.stopPropagation(); setDeleteTarget({ type: 'category', item: cat }) }}
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        <div className="flex items-center justify-between px-6 py-4 border-b">
          ...
          <Button onClick={() => setProdForm({ open: true, initial: { category_id: selectedCat ?? undefined } })}>
        ...
        {loading ? (
          ...
          <EmptyState
            ...
            action={<button className="rounded-lg bg-primary px-3 py-1.5 text-sm text-primary-foreground" onClick={() => setProdForm({ open: true, initial: { category_id: selectedCat ?? undefined } })}>Add Product</button>}
          />
        ) : (
                ...
                <Card key={p.id} className={`p-4 ${!p.active ? 'opacity-50' : ''}`}>
                  <div className="flex items-start justify-between">
                    ...
                    <div className="flex flex-col items-end gap-1 ml-2">
                      ...
                      {(p.cost ?? 0) > 0 && <span className="text-xs text-muted-foreground">Cost: {fmt(p.cost ?? 0)}</span>}
                    </div>
                  </div>
                  <div className="flex items-center justify-between mt-3 pt-3 border-t">
                    <div className="flex items-center gap-2">
                      {p.track_inventory && (
                        <Badge variant={(p.stock_quantity ?? 0) <= 5 ? 'destructive' : 'secondary'} className="text-xs">
                    ...
                    <div className="flex gap-1">
                      <button onClick={() => toggleActive(p)} className="p-1 rounded hover:bg-accent text-xs text-muted-foreground">
                        {p.active ? 'Hide' : 'Show'}
                      </button>
                      <button onClick={() => setProdForm({ open: true, initial: p })} className="p-1.5 rounded hover:bg-accent">
                        <Pencil className="h-3.5 w-3.5" />
                      </button>
                      <button onClick={() => setDeleteTarget({ type: 'product', item: p })}
      ...
      {/* Forms */}
      {catForm.open && <CategoryForm initial={catForm.initial} onSave={handleSaveCategory} onClose={() => setCatForm({ open: false })} />}
      {prodForm.open && <ProductForm initial={prodForm.initial} categories={categories} onSave={handleSaveProduct} onClose={() => setProdForm({ open: false })} />}
      {deleteTarget && (
        <ConfirmDialog
          open
