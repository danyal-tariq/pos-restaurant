          ...
          <Button
            onClick={() => {
              ...
              onSave({
                name,
                role,
                hourly_rate: hourly ? parseFloat(hourly) : 0,
                ...(pin ? { pin } : {})
              })
            }}
          >
...
export function EmployeesPage(): JSX.Element {
  ...
  const [empForm, setEmpForm] = useState<{ open: boolean; initial?: Partial<Employee> }>({
    open: false
  })
  ...
  return (
    <div className="flex flex-col h-full p-6 gap-4">
            ...
            {employees.map((emp) => (
              <Card key={emp.id} className="p-5 flex items-start gap-4">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <Badge className={`mt-1 text-xs ${roleColor(emp.role)}`} variant="outline">
                    {emp.role}
                  </Badge>
                  {(emp.hourly_rate ?? 0) > 0 && (
                ...
                <div className="flex gap-1">
                  <button
                    onClick={() => setEmpForm({ open: true, initial: emp })}
                  ...
                  return (
                    <tr key={shift.id} className="border-t hover:bg-muted/30">
                      ...
                      <td className="px-4 py-3 font-semibold">
                        {fmt((shift.total_cash ?? 0) + (shift.total_card ?? 0))}
                      </td>
      ...
      {empForm.open && (
        <EmployeeForm
          initial={empForm.initial}
          onSave={handleSave}
          onClose={() => setEmpForm({ open: false })}
        />
      )}
      {deleteTarget && (
        <ConfirmDialog
