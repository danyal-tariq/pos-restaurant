    ...
    <div className="flex flex-col h-full p-6 gap-4">
          ...
          {loading ? (
            ...
            <table className="w-full text-sm">
              <thead className="bg-muted/50 sticky top-0">
                <tr>
                  {['Order #', 'Time', 'Type', 'Customer', 'Items', 'Total', 'Status', ''].map(
                    (h) => (
                      <th
                        key={h}
          ...
          <div className="p-6 space-y-5">
            <div className="flex flex-wrap gap-4 text-sm">
              ...
              <div>
                ...
                {orderTypeLabel(selected.order_type)}
              </div>
            ...
            <table className="w-full text-sm border rounded-lg overflow-hidden">
              <thead className="bg-muted/50">
                <tr>
                  {['Item', 'Qty', 'Unit Price', 'Total'].map((h) => (
                    <th key={h} className="text-left px-3 py-2 font-semibold text-muted-foreground">
                      {h}
                    </th>
                  ))}
                </tr>
              ...
              <tbody>
                {(selected.items ?? []).map((item, i) => {
                  const mods: { name: string }[] = item.modifiers_json
                    ? (() => {
                        try {
                  ...
                  return (
                    <tr key={i} className="border-t">
                      <td className="px-3 py-2">
                        ...
                        {item.notes && (
                          <p className="text-xs italic text-orange-600">{item.notes}</p>
                        )}
                      </td>
      ...
      {payTarget && (
        <Modal
          ...
          title={`Collect Payment — ${payTarget.order_number}`}
          size="sm"
        >
          <div className="p-6 space-y-5">
